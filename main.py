    ...
    for event in pygame.event.get():
      ...
      # Обработка событий нажатия клавиш клавиатуры
        ...
        if event.key == pygame.K_r:
          if start_time != None:
            is_confirmed =  confirmBox("Вы уверены, что хотите начать новую симуляцию, прервав текущую?", "Подтверждение")
          else:
            is_confirmed = True
          if is_confirmed:
            cells = np.zeros((60, 80))
        ...
        if event.key == pygame.K_F1:
          infoBox("Управление\n\nSPACE - пауза\nЛКМ - выставление клетки (во время паузы)\nПКМ - удаление клетки (во время паузы)\nR - перезапуск симуляции\nG - случайная генерация клеток\nI - просмотреть информацию о текущей симуляции\nS - включение/выключение замедленного режима\nЦифры (от 1 до 8) - смена цветовой темы\n\n\nЦветовые темы\n\n1 - Чёрный фон, белые клетки\n2 - Чёрный фон, красные клетки\n3 - Чёрный фон, зелёные клетки\n4 - Чёрный фон, синие клетки\n5 - Белый фон, чёрные клетки\n6 - Белый фон, красные клетки\n7 - Белый фон, зелёные клетки\n8 - Белый фон, синие клетки", "Помощь", "info")
    ...
    if running:
      ...
      if len(np.nonzero(cells)[0]) == 0 and running:
        ...
        if start_time != None and generation != 1:
          ...
          time_passed = "0 секунд"
        infoBox(f"[Симуляция прекращена]\n(все клетки вымерли)\n\nПоколение: {generation}\nЖивых клеток: {alive}\nПрошло времени: {time_passed}\n\nКлеток добавлено: {cells_added}\nКлеток удалено: {cells_deleted}\nКлеток сгенерировано случайно: {cells_randomly_generated}\nКлеток оставшихся после предыдущей симуляции: {cells_after_last_simulation}", "Информация", "info")
        generation = 0
        start_time = None
        prev_cells = np.zeros((60, 80))
        preprev_cells = np.zeros((60, 80))
